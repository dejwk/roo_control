#include "roo_control/transceivers/remote/server.h"
...
size_t TransceiverDescriptorHashFn::operator()(
    const roo_control_TransceiverDescriptor& descriptor) const {
  size_t hash = 0;
...
bool TransceiverDescriptorEq::operator()(
    const roo_control_TransceiverDescriptor& a,
...
TransceiverUniverseServer::TransceiverUniverseServer(
    TransceiverUniverse& universe, TransceiverUniverseServerChannel& channel,
  ...
  channel.registerClientMessageCallback(
      [this](const roo_control_TransceiverClientMessage& msg) {
...
void TransceiverUniverseServer::devicesChanged() {
  {
...
void TransceiverUniverseServer::newReadingsAvailable() {
  {
    std::lock_guard<std::mutex> lock(state_guard_);
    if (transmission_in_progress_) {
...
void TransceiverUniverseServer::handleClientMessage(
    const roo_control_TransceiverClientMessage& msg) {
  switch (msg.which_contents) {
    case roo_control_TransceiverClientMessage_request_update_tag: {
      universe_.requestUpdate();
      break;
    }
    case roo_control_TransceiverClientMessage_request_state_tag: {
      handleRequestState();
      break;
    }
    case roo_control_TransceiverClientMessage_write_tag: {
      const auto& req = msg.contents.write;
      TransceiverActuatorLocator loc(req.device_locator_schema,
                                     req.device_locator_id,
...
void TransceiverUniverseServer::handleRequestState() {
  {
    std::lock_guard<std::mutex> lock(state_guard_);
    if (transmission_in_progress_) {
...
void TransceiverUniverseServer::triggerTransmission() {
  transmit_executor_.execute([this]() { transmissionLoop(); });
}

void TransceiverUniverseServer::State::clearAll() {
  clearDelta();
  devices_.clear();
  descriptors_.clear();
...
void TransceiverUniverseServer::State::clearDelta() {
...
void TransceiverUniverseServer::State::newSensorReadingDelta(
    const roo_control::TransceiverSensorLocator& loc, float value,
...
void TransceiverUniverseServer::snapshotDevices() {
  roo_control_TransceiverDescriptor descriptor;
  roo_collections::FlatSmallHashSet<roo_control::TransceiverDeviceLocator>
      removed(state_.device_count());
  for (const auto& itr : state_.devices()) {
    removed.insert(itr.first);
  }
  int ordinal = 0;
  universe_.forEachDevice(
      [&](const roo_control::TransceiverDeviceLocator& loc) -> bool {
        ...
        if (existing == state_.devices().end()) {
          ...
          TransceiverDescriptorEq descriptor_eq;
          if (descriptor_eq(old_descriptor, descriptor)) {
...
void TransceiverUniverseServer::snapshotSensorState() {
  ...
  for (const auto& dev : state_.device_deltas()) {
    ...
    for (size_t i = 0; i < descriptor.sensors_count; i++) {
      TransceiverSensorLocator sensor_locator(dev.locator,
                                              descriptor.sensors[i].id);
      roo_control::Measurement measurement = universe_.read(sensor_locator);
...
int TransceiverUniverseServer::State::addDescriptorReference(
    const roo_control_TransceiverDescriptor& descriptor) {
  int key;
  bool is_new = false;
...
void TransceiverUniverseServer::State::removeDescriptorReference(
    const roo_control_TransceiverDescriptor& descriptor) {
  auto itr = descriptors_.find(descriptor);
  if (itr == descriptors_.end()) {
...
void TransceiverUniverseServer::transmitUpdateBegin(bool delta) {
  roo_control_TransceiverServerMessage msg =
      roo_control_TransceiverServerMessage_init_zero;
  msg.which_contents =
      roo_control_TransceiverServerMessage_transceiver_update_begin_tag;
  msg.contents.transceiver_update_begin.delta = delta;
  channel_.sendServerMessage(msg);
}

void TransceiverUniverseServer::transmitUpdateEnd() {
  roo_control_TransceiverServerMessage msg =
      roo_control_TransceiverServerMessage_init_zero;
  msg.which_contents =
      roo_control_TransceiverServerMessage_transceiver_update_end_tag;
  channel_.sendServerMessage(msg);
}

void TransceiverUniverseServer::transmitDescriptorAdded(int key) {
  const roo_control_TransceiverDescriptor& descriptor =
      state_.descriptors_by_key()[key];
  {
    roo_control_TransceiverServerMessage msg =
        roo_control_TransceiverServerMessage_init_zero;
    msg.which_contents =
        roo_control_TransceiverServerMessage_descriptor_added_tag;
...
void TransceiverUniverseServer::transmitDescriptorRemoved(int key) {
  roo_control_TransceiverServerMessage msg =
      roo_control_TransceiverServerMessage_init_zero;
  msg.which_contents =
      roo_control_TransceiverServerMessage_descriptor_removed_tag;
  msg.contents.descriptor_removed.key = key;
  channel_.sendServerMessage(msg);
}

void TransceiverUniverseServer::transmitDeviceAdded(
    const TransceiverDeviceLocator& locator, int descriptor_key) {
  roo_control_TransceiverServerMessage msg =
      roo_control_TransceiverServerMessage_init_zero;
  msg.which_contents = roo_control_TransceiverServerMessage_device_added_tag;
...
void TransceiverUniverseServer::transmitDevicesPreserved(
    int first_preserved_ordinal, size_t count) {
  roo_control_TransceiverServerMessage msg =
      roo_control_TransceiverServerMessage_init_zero;
  msg.which_contents =
      roo_control_TransceiverServerMessage_device_preserved_tag;
...
void TransceiverUniverseServer::transmitDeviceModified(int prev_ordinal,
                                                       int descriptor_key) {
  roo_control_TransceiverServerMessage msg =
      roo_control_TransceiverServerMessage_init_zero;
  msg.which_contents = roo_control_TransceiverServerMessage_device_modified_tag;
...
void TransceiverUniverseServer::transmitDeviceRemoved(int prev_ordinal) {
  roo_control_TransceiverServerMessage msg =
      roo_control_TransceiverServerMessage_init_zero;
  msg.which_contents = roo_control_TransceiverServerMessage_device_removed_tag;
  auto& payload = msg.contents.device_removed;
  payload.prev_index = prev_ordinal;
  channel_.sendServerMessage(msg);
}

void TransceiverUniverseServer::transmitReadingsBegin() {
  roo_control_TransceiverServerMessage msg =
      roo_control_TransceiverServerMessage_init_zero;
  msg.which_contents = roo_control_TransceiverServerMessage_readings_begin_tag;
  channel_.sendServerMessage(msg);
}

void TransceiverUniverseServer::transmitReadingsEnd() {
  roo_control_TransceiverServerMessage msg =
      roo_control_TransceiverServerMessage_init_zero;
  msg.which_contents = roo_control_TransceiverServerMessage_readings_end_tag;
  channel_.sendServerMessage(msg);
}

void TransceiverUniverseServer::transmissionLoop() {
  bool is_delta;
  {
    std::lock_guard<std::mutex> lock(state_guard_);
...
void TransceiverUniverseServer::transmit(bool is_delta) {
  ...
  if (!state_.reading_delta_groups().empty()) {
    ...
    for (const auto& group : state_.reading_delta_groups()) {
      roo_control_TransceiverServerMessage msg =
          roo_control_TransceiverServerMessage_init_zero;
      msg.which_contents = roo_control_TransceiverServerMessage_reading_tag;
